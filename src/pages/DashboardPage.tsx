...
export function DashboardPage() {
  ...
  // TODO: Zustand + useSearchParams를 통한 파이프라인 상태 관리
  ...
  // Frontier: first non-completed stage index
  const frontierIndex = (() => {
  ...
  return (
    <div className="h-screen bg-slate-50 flex overflow-hidden">
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-5 flex flex-col gap-1">
          ...
          {PIPELINE_STAGES.map((stage, index) => {
            ...
            return (
              <button
                key={stage.id}
                onClick={() => !isLocked && setActiveStep(stage.id)}
                disabled={isLocked}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Header */}
        <header className="bg-white border-b border-slate-100 px-8 py-4 flex items-center justify-between gap-4 shrink-0">
          ...
          <div
            className={`flex items-center gap-2 px-4 py-2 rounded-xl shrink-0 transition-all duration-500 ${
