...
export function CustomerTable({ users, onSelect }: CustomerTableProps) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-white rounded-xl border border-slate-100 overflow-hidden">
      <div className="grid grid-cols-[1.2fr_1.8fr_0.8fr_1.5fr_0.7fr_0.8fr] border-b border-slate-100 py-3 px-4 text-xs font-semibold text-slate-500">
        <div>회사명</div>
        <div>이메일</div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {sorted.map((u) => {
          ...
          return (
            <div
              key={u.id}
              className="grid grid-cols-[1.2fr_1.8fr_0.8fr_1.5fr_0.7fr_0.8fr] items-center py-3 px-4 border-b border-slate-50 hover:bg-slate-50/50 transition-colors cursor-pointer"
            ...
            >
              ...
              <div className="text-center">
                <span
                  className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded ${style.className}`}
                >
                  {style.label}
                </span>
              </div>
